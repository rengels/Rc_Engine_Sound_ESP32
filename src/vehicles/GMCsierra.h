...
volatile int startVolumePercentage = 72; // Adjust the start volume (usually = 80%)
...
#include "sounds/GMCSierraPickupIdle.h" // GMC Sierra V8 Pickup
...
volatile int engineRevVolumePercentage = 70; // the engine volume will be throttle dependent (usually = 40%, never more than 100%!) 60
...
#ifdef REV_SOUND
#include "sounds/GMCSierraPickupRev.h" // GMC Sierra V8 Pickup
#endif
...
volatile int dieselKnockVolumePercentage = 60; // Adjust the Diesel knock volume (usually = 40 - 120%) 140
...
#include "sounds/GMCSierraPickupKnock.h" // GMC Sierra V8 Pickup
...
volatile int sirenVolumePercentage = 48; // Adjust the siren volume (usually = 80%)
